...
class ConnectFourGUI:
    ...
    def animate_drop(self, col: int, color: Tuple[int, int, int]):
        ...
        for row in range(self.game.rows + 1):
            # Clear animation row
            pygame.draw.rect(
                self.screen, self.WHITE, (0, 0, self.width, self.SQUARESIZE)
            )
    ...
    def get_ai_move(self) -> int:
        ...
        action, _ = self.mcts.search(model_state, total_iterations=100, temperature=0.1)
        return action
        ...
        while not game_over:
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEMOTION and not game_over:
                    ...
                    pygame.draw.circle(
                        self.screen, color, (pos_x, self.SQUARESIZE // 2), self.RADIUS
                    )
                    pygame.display.update()
